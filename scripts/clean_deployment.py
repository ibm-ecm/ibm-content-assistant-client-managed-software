...
###############################################################################
'''
Script to clean IBM Content Assistant Operator and deployments
...
def version_callback(value: bool):
    if value:
        print(f"IBM Content Assistant Cleanup -- CLI: {__version__}")
        raise typer.Exit()
...
def display_mode_version(mode: str, description: str):
    ...
    print(Panel.fit(msg, title="IBM Content Assistant Cleanup -- CLI", border_style="green"))
    print()
...
def operator():
    """
        Uninstall IBM Content Assistant Operator Only.
    """
    operator_dict = state["clean"].collect_operator_details()
    if not operator_dict:
        print()
        print(Panel.fit("IBM Content Assistant Operator not found in {namespace}".format(
    ...
    if not state["silent"]:
        clean_deployment = Confirm.ask(
            "Do you want to proceed and cleanup the above IBM Content Assistant Operator?")
    else:
    ...
    if clean_deployment:
        ...
        print(Panel.fit("Starting IBM Content Assistant Operator Cleanup", style="cyan"))
        with Progress(
                SpinnerColumn(),
            ...
            task1 = progress.add_task("[green]Uninstalling IBM Content Assistant Operator", total=operator_task_num)
...
def deployment():
    """
        Uninstall Content Assistant Deployment Only.
    """
    ...
    if not deployment_dict:
        print()
        print(Panel.fit(
            "IBM Content Assistant Deployment not found in {namespace}".format(
    ...
    if not state["silent"]:
        clean_deployment = Confirm.ask(
            "Do you want to proceed and cleanup the above IBM Content Assistant Deployment?")
    else:
        clean_deployment = True
    ...
    if clean_deployment:
        ...
        print(Panel.fit("Starting IBM Content Assistant Deployment Cleanup", style="cyan"))
        with Progress(
                SpinnerColumn(),
                TextColumn("[progress.description]{task.description}"),
                BarColumn(),
            ...
            task1 = progress.add_task("[yellow]Cleaning IBM Content Assistant Deployment", total=7)
...
def main(ctx: typer.Context,
         ...
         dryrun: Annotated[bool, typer.Option(
             ...
             rich_help_panel="Customization and Utils")] = False):
    """
        IBM Content Assistant Deployment Cleanup CLI.
    """
    if verbose:
        state["verbose"] = True
    ...
    if ctx.invoked_subcommand is None:
        display_mode_version("Deployment and Operator Cleanup",
                             "Clean up of the IBM Content Assistant Operator and Deployment")

    elif ctx.invoked_subcommand == "operator":
        display_mode_version("Operator Cleanup", "Clean up of the IBM Content Assistant Operator Only")
    ...
    if ctx.invoked_subcommand is None:
        ...
        if not operator_dict and not deployment_dict:
            print()
            print(Panel.fit("IBM Content Assistant Operator or Deployment not found in {namespace} namespace".format(
                namespace=state["clean"]._deployment_prerequisites.namespace), style="red"))
            exit(1)
        cleanup_summary = display_deployment_resources(logger=state['logger'],
                                                       deployment_resources=resource_dict,
                                                       deployment_details=deployment_dict,
        ...
        if not state["silent"]:
            if operator_dict and deployment_dict:
                msg = "Do you want to proceed and cleanup the above IBM Content Assistant Deployment and Operator?"
            elif operator_dict:
                msg = "Do you want to proceed and cleanup the above IBM Content Assistant Operator?"
            else:
                msg = "Do you want to proceed and cleanup the above IBM Content Assistant Deployment?"
        ...
        if clean_deployment:
            ...
            print(Panel.fit("Starting IBM Content Assistant Deployment and Operator Cleanup", style="cyan"))
            with Progress(
                    SpinnerColumn(),
                    TextColumn("[progress.description]{task.description}"),
                    BarColumn(),
                ...
                if operator_dict:
                    task1 = progress.add_task("[green]Uninstalling IBM Content Assistant Operator", total=operator_task_num)
                if deployment_dict:
                    task2 = progress.add_task("[yellow]Cleaning IBM Content Assistant Deployment", total=7)
