...
class CleanDeployment:
    ...
    def collect_cr_details(self):
        ...
        self._cr_name = cr_details.get("name", 'casdeploy')
        version = cr_details.get("version", "1.0.0")
    ...
    def delete_CR(self, task1, progress):
        ...
        progress.log("Starting IBM Content Assistant Deployment Cleanup")
        progress.log()
        ...
        self._custom_api.delete_namespaced_custom_object(group="cas.ibm.com", version="v1",
                                                         namespace=self._namespace,
                                                         plural="casclusters", name=self._cr_name)
        ...
        try:
            ...
            while retries < 20:
                ...
                if all_pods_deleted:
                    ...
                    break
            if retries == 20:
                progress.log(Text("Timeout Waiting for Clean up of IBM Content Assistant Deployment pods\n"
    ...
    def delete_operator(self, task1, progress):
        ...
        if self._deployment_prerequisites.platform.lower() in ["ocp"]:
            ...
            progress.log("Starting IBM Content Assistant Operator OLM Uninstallation")
            progress.log()
            self._logger.info(f"Starting IBM Content Assistant Operator OLM uninstallation")
            time.sleep(SLEEP_TIMER)
            ...
            if provided_apis and len(provided_apis) > 1:
                ...
                progress.log()
                progress.log("Updating Operator Group to remove the Operator API served by IBM Content Assistant Operator")
                self._logger.info("Updating Operator Group to remove the Operator API served by IBM Content Assistant Operator")
            ...
            progress.log()
            progress.log(Panel.fit("Uninstalling IBM Content Assistant Operator completed!", style="bold green"))
            progress.log()
            self._logger.info(f"IBM Content Assistant Operator uninstallation completed!")
            progress.advance(task1)
            ...
            progress.log("Starting IBM Content Assistant Operator Yaml Uninstallation")
            progress.log()
            self._logger.info(f"Starting IBM Content Assistant Operator yaml uninstallation")
            time.sleep(SLEEP_TIMER)
