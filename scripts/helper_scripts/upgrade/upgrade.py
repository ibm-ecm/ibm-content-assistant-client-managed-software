        ...
        try:
            ...
            while retries < 40:
                ...
                running_pods = [pod.metadata.name for pod in pods.items if
                                "ibm-content-assistant-catalog" in pod.metadata.name and pod.status.phase == "Running"]
                if running_pods:
                    progress.log(
            ...
            if retries == 40:
                ...
                progress.log()
                progress.log(Syntax(
                    f"kubectl describe pod $(kubectl get pod -n {self._catalog_namespace} | grep ibm-content-assistant-catalog | awk '{{print $1}}') -n ${self._catalog_namespace}",
                    "bash"))
    ...
    def wait_for_operator(self, progress, task):
        try:
            ...
            while retries < 40:
                ...
                running_pods = [pod.metadata.name for pod in pods.items if
                                "ibm-content-assistant-operator" in pod.metadata.name and "catalog" not in pod.metadata.name and pod.status.phase == "Running" and
