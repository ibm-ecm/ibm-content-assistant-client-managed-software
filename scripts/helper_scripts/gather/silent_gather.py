...
class SilentGatherOptions(GatherOptions):
    ...
    def silent_mustgather_components(self):
        genai_connector = gather_var(key="GENAI_CONNECTOR", _logger=self._logger, _envfile=self._envfile,
                                     _error_list=self._error_list)

        if genai_connector is not None and genai_connector is True:
            super().components.add("genai_connector")
    ...
    def silent_version(self):
        version = gather_var(key="CAS_VERSION", valid_values=[1], _logger=self._logger,
