...
class GatherOptions:
    ...
    def collect_mustgather_components(self, component_list, version="1.0.0"):
        try:
            ...
            print("In addition to the deployment artifacts, the IBM Content Assistant MustGather can collect components specific logs and configuration files.\n"
            ...
            while True:
                print()
                print("Select zero or more IBM Content Assistant Components")
                print("Enter a number to toggle selection")
                print("Enter [[b]0[/b]] to finish selection")
                print(f'1. GenAI Connector {":heavy_check_mark:" if 1 in choices else ""}')
                print()

                result = IntPrompt.ask(f'Enter a valid option [[b]0[/b] and [b]{num_components}[/b]]')
                ...
                if 1 <= result <= num_components:
                    ...
                    print()
                    print(
                        "In addition to the deployment artifacts, the IBM Content Assistant MustGather can collect components specific logs and configuration files.\n"
                        "This includes the following components:\n\n"
                        "- Component Logs\n"
                        "- Component Version\n"
                        "- Python Version\n")
                else:
                    print(f'[prompt.invalid]Number must be between [[b]0[/b] and [b]{num_components}[/b]]')
    ...
    def collect_sensitive_data(self, collect=None):
        try:
            if collect is None:
                ...
                print()
                print("The IBM Content Assistant MustGather would like to collect sensitive configuration data to help diagnose and troubleshoot issues.\n"
    ...
    def collect_private_catalog(self):
        ...
        if self._platform != "other":
            ...
            self._private_catalog = not (Confirm.ask(
                "Do you want to deploy IBM Content Assistant Operator using a global catalog?", default=False))
    ...
    def display_preupgrade_steps(self):
        ...
        while True:
            print(
                f"Please see IBM Content Assistant Documentation for important upgrade prerequisites: {upgrade_link}")
            self._preupgrade_steps = Confirm.ask(
                "Have you completed IBM Content Assistant Operator upgrade prerequisites")
