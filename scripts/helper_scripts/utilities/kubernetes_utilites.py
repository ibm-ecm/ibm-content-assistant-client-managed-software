...
class KubernetesUtilities:
    ...
    def calculate_user_configmaps(self, components=list):
        ...
        # Collect Custom Config from content_assistant_configuration

        try:
            if "content_assistant_configuration" in cr_keys:
                if cr["spec"]["content_assistant_configuration"].get("content_assistant_production_settings"):
                    if cr["spec"]["content_assistant_configuration"]["content_assistant_production_settings"].get("custom_configmap"):
                        for item in cr["spec"]["content_assistant_configuration"]["content_assistant_production_settings"]["custom_configmap"]:
                            if "name" in item.keys():
                                configmaps.add(item["name"])
            else:
                self._logger.info(f"No IBM Content Assistant Configmaps found")
    ...
    def calculate_user_secrets(self, components=list):
        ...
        # - content_assistant_secret_name
        # - vector_database_secret_name
        # - access_token_key_pem_secret
        # - vector_database_certificate

        try:

            # Collect content assistant secret
            secrets.add(cr["spec"]["content_assistant_configuration"].get("content_assistant_secret_name", 'ibm-content-assistant-secret'))

            # Collect vector database secret
            secrets.add(cr["spec"]["vector_database"].get("vector_database_secret_name",
                                                                          'vector-database-secret'))
            # Collect API Key secret
            # This is an internal secret used by the Content Assistant GenAI Connector
            secrets.add("ibm-content-assistant-api-keys-secret")

            # If vector database is SSL enabled, collect the certificate secret
            if cr["spec"]["vector_database"].get("vector_database_ssl_enabled", False):
                secrets.add(cr["spec"]["vector_database"].get("vector_database_certificate", 'vector-database-certificate-secret'))

            # Collect access token key PEM secret
            secrets.add(cr["spec"]["content_assistant_configuration"]["content_assistant_production_settings"]['cli'].get("access_token_key_pem_secret", 'ibm-content-assistant-admin-key-secret'))

        except Exception as e:
            self._logger.info(f"No IBM Content Assistant Secrets found")
    ...
    def calculate_deployed_components(self):
        try:
            ...
            # Only one type of component is deployed in 1.0.0

            # Collect different sections from the CR
            components.add("genai_connector")
    ...
    def parse_cr(self):
        try:
            ...
            cr_details = {
                ...
                "context": cr["spec"]["shared_configuration"]["sc_cas_license_type"],
            ...
            # Equate AppVersion to CAS Version
            versions = {
                "1.0.0": "1.0.0",
            }
    ...
    # Function to list the resources deployed by Content Assistant deployment in a specific namespace
    # Can see to add more resource types
    def list_namespace_resources(
            self, console, namespace, platform, filter="casdeploy"
    ):
        try:
    ...
    def describe_catalogsource(self, name="ibm-content-assistant-operator-catalog", namespace=""):
    ...
    def get_subscription(self, namespace):
        try:
            ...
            for subscription in subscriptions.get('items', []):
                if "ibm-content-assistant-operator" in subscription["metadata"]['name']:
    ...
    def delete_operator_cncf(self, namespace=""):
        ...
        name = "ibm-content-assistant-operator"
        try:
    ...
    def delete_role_binding(self, namespace="", name=""):
        if not name:
            name = "ibm-content-assistant-operator-rb"
        try:
            # Delete the RoleBinding
            self._rbac_v1.delete_namespaced_role_binding(name, namespace)
        except Exception as e:
    ...
    def delete_role(self, namespace="", name=""):
        if not name:
            name = "ibm-content-assistant-operator-role"
        try:
            # Delete the Role
            self._rbac_v1.delete_namespaced_role(name, namespace)
        except Exception as e:
    ...
    def delete_service_account(self, namespace="", name=""):
        if not name:
            name = "ibm-content-assistant-operator-sa"
        try:
            # Delete the ServiceAccount
            self._core_v1.delete_namespaced_service_account(name, namespace)
        except Exception as e:
    ...
    def apply_role_binding(self, resource_file, namespace):
        ...
        while True:
            try:
                role = self._rbac_v1.read_namespaced_role(name="ibm-content-assistant-operator-role", namespace=namespace)
                if role.metadata.name == 'ibm-content-assistant-operator-role':
    ...
    def get_operator_group(self, namespace):
        ...
        try:
            ...
            for group in og.get('items', []):
                if "CASCluster" in group["metadata"]['annotations']['olm.providedAPIs']:
    ...
    def get_rolename(self, namespace):
        try:
            ...
            for role in roles.items:
                if "ibm-content-assistant-op" in role.metadata.name:
    ...
    def get_rolebinding(self, namespace):
        try:
            ...
            for rolebind in rolebindings.items:
                if "ibm-content-assistant-op" in rolebind.metadata.name:
    ...
    def get_service_account(self, namespace):
        try:
            ...
            for sa in service_accounts.items:
                if "ibm-content-assistant-operator-sa" in sa.metadata.name:
    ...
    def parse_operator_deployment(self, deployment, namespace):
        try:
            ...
            operator_deployment_details = {"deployment": name, "namespace": namespace,
                                           ...
                                           "type": "YAML",
                                           "release": deployment.spec.template.metadata.labels.get("release", "1.0.0")}
    ...
    def get_operator_details(self, namespace, deployment_name="ibm-content-assistant-operator"):
        try:
    ...
    def copy_files_from_pod(self, pod_name, namespace, src_path, dest_path):
        try:
            exec_command = ["tar", "czf", '-', "-C", src_path, '.' ]
    ...
    def update_operator_group(self, namespace, operator_group, provided_apis, api_to_remove="CASCluster.v1.cas.ibm.com"):
    ...
    def get_deployment_cr(self, namespace, logger=None):
        ...
        try:
            cr_details = self._custom_api.list_namespaced_custom_object(group="cas.ibm.com", version="v1",
                                                                        namespace=namespace,
                                                                        plural="casclusters")
            self._logger.info(f"CR details in namespace '{namespace}': {cr_details}")
        ...
        if scale == "down":
            try:
                for deployment in deployments:
                    ...
                    if deployment.metadata.name != "ibm-content-assistant-operator":
                        self._apps_v1.patch_namespaced_deployment_scale(
                            name=deployment.metadata.name,
                            namespace=namespace,
                            body={"spec": {"replicas": 0}}
