...
def selection_tree(selection_summary: dict) -> Tree:
    ...
    version_tree = Tree("IBM Content Assistant Version")
    version_tree.add(selection_summary["cas_version"])
    ...
    vector_database_tree = Tree("Vector Database Details")
    for vector_database in selection_summary["vector_database_details"]:
        vector_database_label_tree = Tree("Vector Database Label")
        vector_database_label_tree.add(str(vector_database.get_db_label))
        auth_tree = Tree("Vector Database Authentication Type")
        auth_tree.add(str(vector_database.get_db_auth_type))
        #vector_database_label_tree.add(auth_tree)
        vector_database_tree.add(vector_database_label_tree)
        vector_database_tree.add(auth_tree)
    #vector_database_tree.add(selection_summary["vector_database_authentication_type"])

    tree.add(vector_database_tree)
...
def mustgather_details(cr_details: dict, components: [], operator_details: dict) -> Layout:
    ...
    summary_msg = (
        "\nThe IBM Content Assistant MustGather collections a set of files that will help the IBM support team diagnose"
    ...
    if not operator_details:
        ...
        summary_msg += ("- Content Assisant Operator Deployment & Logs\n"
    ...
    if not cr_details:
        ...
        summary_msg += ("- The IBM Content Assistant Custom Resource (CR) file\n"
                        "- Components Logs\n"
...
def upgrade_details(deployment_details: dict, version_details: dict, current_operator: dict) -> Layout:
    ...
    summary_msg = ("\nThe IBM Content Assistant Operator Deployment will upgrade\n"
    ...
    if deployment_details:
        ...
        summary_msg += ("- IBM Content Assistant Operator Deployment\n"
                        "- Role, RoleBinding & Service Account\n"
                        "- IBM Content Assistant Custom Resource Definition (CRD)\n")
        ...
        if deployment_details["type"] == "OLM":
            ...
            summary_msg += ("- IBM Content Assistant Operator Catalog Source\n"
...
def deploy_details(deployment_details: dict, version_details: dict) -> Layout:
    ...
    summary_msg = ("\nThe IBM Content Assistant Operator Deployment will apply\n"
                   "the following artifacts to your cluster:\n\n")
...
def generate_gather_results(property_folder: str, selection_summary: dict) -> Layout:
    ...
    instructions = Panel.fit(
        ...
        f"3. If SSL is enabled for the Vector Database(s), add the certificate(s) to ./propertyFile/{namespace}/ssl-certs/<vector_database_label>\n"
        "4. All SSL and trusted certificates need to be in PEM (Privacy Enhanced Mail) format\n"
        "5. Run the following command to generate secrets and CR file\n".strip()
    )
    command = Panel.fit(
...
def upgrade_deployment_details(update_list: list, version_details: dict, download_folder_path: ""):
    right_panels = []
    msg = Text("IBM Content Assistant Deployment Preparation", style="bold green")
    msg_panel = Panel.fit(msg)

    if version_details["version"] in ("5.7.0"):
        msg = f"The IBM Content Assistant Custom Resource (CR) has been upgraded.\n\n" \
    ...
    if download_folder_path:
        cr_tree = print_directory_tree("CASUpgrade", download_folder_path)
...
def display_airgap_vars(airgap_vars=None):
    ...
    airgap_msg = Text(
                      "\n\nThe following variables will be used to configure and mirror the IBM Content Assistant Images."
...
def display_issues(generate_folder=None, required_fields=None,
    ...
    section_files = ['cas_vector_database.toml', 'cas_properties.toml']
    unsectioned_files = ['cas_deployment.toml', 'cas_ingress.toml']
...
def generate_generate_results(generate_folder: str) -> Layout:
    ...
    instructions = Panel.fit(
        ...
        "2. Run the following command to validate"
    )
...
def generate_loadimage_results(summary: {}) -> Layout:
    ...
    instructions = Panel.fit(
        ...
        "2. To configure your IBM Content Assistant deployment to use the private registry set the following in your Custom Resource File"
    )
...
def mustgather_network_results(networkpolicy_folder: str, namespace='<namespace>') -> Layout:
    ...
    right_panel_list.append(print_directory_tree("CASNetworkPolicies", networkpolicy_folder))
...
def generate_loadimages_results(imageDetailFolder: str, airgap=False) -> Layout:
    ...
    if airgap:
        instructions = Panel.fit(
            ...
            "2. The IBM Content Assistant CASE Package has been downloaded to your current directory\n"
...
def display_deployment_resources(logger, deployment_resources=None, deployment_details=None, operator_details=None, version_details=None):
    ...
    message = Text("IBM Content Assistant Deployment Resources to be Deleted", style="bold blue", justify="center")
    result_panel = Panel(message)
    layout["upper"].update(result_panel)
    ...
    if deployment_details:
        ...
        instructions_msg += ("\n\nIBM Content Assistant Deployment: \n"
                             "  - Deployments\n"
                             "  - Services\n"
        ...
        deployment_missing = Panel.fit("No IBM Content Assistant Deployment Found", style="bold red")
        left_panel_list.append(deployment_missing)

    # Operator Details Panel
    if operator_details:
        instructions_msg += ("\n\nIBM Content Assistant Operator: \n"
                             "  - Operator Deployment\n"
                             "  - Role, RoleBinding & Service Account\n")
