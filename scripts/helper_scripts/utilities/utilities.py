...
def read_version_toml(file_path, logger):
    try:
        ...
        display = version_data.get("VERSION", "1.0.0")
...
def replace_namespace_in_file(project_name, input_file, output_file, resource_type="", private=False):
    ...
    if resource_type.lower() == "cluster role binding":
        ...
        replaced_content = re.sub(r'name: .*', f"name: ibm-content-assistant-operator", replaced_content)
        if private:
...
def create_deployment_info(setup, version_data):
    ...
    if setup.private_catalog:
        ...
        catalog_type = "Global"
    catalog_source = "ibm-content-assistant-operator-catalog"

    deployment_details = {
        "deployment": "ibm-content-assistant-operator",
        "release": version,
        "type": type,
        "installedCSV": csv,
        "channel": channel,
    ...
    if len(path) == 1:
        if requests:
            try:
                ...
                if resource_limits_comparison(dictionary1[key]["requests"]["memory"],
                                              dictionary2[key]["requests"]["memory"]):
                    dictionary1[key]["requests"]["memory"] = dictionary2[key]["requests"]["memory"]
                if resource_limits_comparison(dictionary1[key]["requests"]["ephemeral_storage"],
                                              dictionary2[key]["requests"]["ephemeral_storage"]):
            ...
            try:
                if resource_limits_comparison(dictionary1[key]["limits"]["cpu"], dictionary2[key]["limits"]["cpu"],
                                              limits=True):
                    dictionary1[key]["limits"]["cpu"] = dictionary2[key]["limits"]["cpu"]
                if resource_limits_comparison(dictionary1[key]["limits"]["memory"],
                                              dictionary2[key]["limits"]["memory"], limits=True):
                    dictionary1[key]["limits"]["memory"] = dictionary2[key]["limits"]["memory"]
                if resource_limits_comparison(dictionary1[key]["limits"]["ephemeral_storage"],
                                              dictionary2[key]["limits"]["ephemeral_storage"], limits=True):
