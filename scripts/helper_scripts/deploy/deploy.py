...
class Deploy:
    ...
    def cluster_setup(self, progress, task):
        ...
        try:
            ...
            self._logger.info("Creating namespace for the IBM Content Assistant Deployment")
            progress.log("Creating namespace for the IBM Content Assistant Deployment")
            progress.log()
    ...
    def apply_cncf(self, progress, task):
        try:
            ...
            self._logger.info("Apply/Patching Service Account")
            progress.log("Apply/Patching Service Account")
            progress.log()
            self._kube.apply_cluster_resource_files(
        ...
        try:
            ...
            while retries < 40:
                ...
                if running_pods:
                    ...
                    retries = retries + 1
                    progress.log(f"IBM Content Assistant Catalog deployment in progress ({retries + 1}/40) ")
                    progress.log()
                    sleep(5)
        ...
        try:
            ...
            while retries < 40:
                ...
                if running_pods:
                    ...
                    retries = retries + 1
                    progress.log(f"IBM Content Assistant Operator deployment in progress ({retries + 1}/40) ")
                    progress.log()
                    sleep(15)

            if retries == 40:
                self._logger.info("Timeout Waiting for IBM Content Assistant Operator pod to start")
                ...
                progress.log()
                progress.log(Syntax(
                    f"kubectl describe pod $(kubectl get pod -n {self._setup.namespace} | grep ibm-content-assistant-operator | awk '{{ print $1 }}') -n {self._setup.namespace}",
                    "bash"))
                exit()

            self._logger.info("IBM Content Assistant Operator Deployment Completed")
            progress.log(Panel.fit("IBM Content Assistant Operator Deployment Completed", style="bold green"))
            progress.update(task, advance=1)
        except Exception as e:
            self._logger.debug(f"Error occurred while applying the resources: {e}")
            progress.log(Text(f"Error occurred while applying the resources: {e}", style="bold red"))
    ...
    def apply_operator_olm(self, progress, task):
        ...
        self._logger.info("Starting IBM Content Assistant Operator Installation")
        progress.log(Panel.fit("Starting IBM Content Assistant Operator Installation", style="cyan"))
        progress.log()
    ...
    def apply_operator_cncf(self, progress, task):
        ...
        if self._setup.entitlement_key_valid:
            self._logger.info("IBM Content Assistant Operator is being installed using the IBM Entitlement Registry")
            progress.log("IBM Content Assistant Operator is being installed using the IBM Entitlement Registry")
            progress.log()
            ...
            self._logger.info("IBM Content Assistant Operator is being installed using a private registry")
            progress.log("IBM Content Assistant Operator is being installed using a private registry")
            progress.log()
            pattern = re.compile(re.escape(registry_in_file) + r'\b')
        ...
        self._logger.info("Applying/Patching IBM Content Assistant Operator Operator Deployment")
        progress.log(f"Applying/Patching IBM Content Assistant Operator Operator Deployment")
        progress.log()
